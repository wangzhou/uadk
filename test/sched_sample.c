...
struct sched_operator {
	...
	int (*poll_policy)(const struct wd_ctx_config *cfg,
...
static int sample_poll_region(const struct wd_ctx_config *cfg,
			      __u32 begin,
			      __u32 end,
			      __u32 expect,
			      __u32 *count)
 ...
 */
handle_t sample_sched_pick_next_ctx(const struct wd_ctx_config *cfg,
				    const void *req,
